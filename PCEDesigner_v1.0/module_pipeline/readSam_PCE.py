...
from annotByPos_PCE import gffJudge 
...
class samReader:
    def __init__(self, genome, hitSeqChrom, hitSeqStart, hitSeqEnd, pam, guide_len, pamInfo_dict, sam):
        self.genome = genome
    ...
    def readFile(self):
        ...
        for l in open(self.sam, "r"):
            ...
            if lst[1]=='4':     # some hit seq have hit to a unplaced ref
                continue
            ...
            if self.chrom == self.hitSeqChrom:
                if self.seqDirect == "+":
                    ...
                    tarPos = self.hitSeqEnd - sgRNA_pos + 1 - self.tarSeq_len + 1
    ...
    def annoteTarget(self, hitGene_info):
        ...
        for tarID in list(self.pamInfo_dict.keys()):
            if self.seqDirect == "+":
                ...
                pos = self.hitSeqEnd - self.pamInfo_dict[tarID][-1] + 1 - self.tarSeq_len + 1
